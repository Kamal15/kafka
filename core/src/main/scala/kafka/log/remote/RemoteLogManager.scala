...
import org.apache.kafka.server.log.remote.storage.{LogSegmentData, RemoteLogManagerConfig, RemoteLogMetadataManager, RemoteLogSegmentId, RemoteLogSegmentMetadata, RemoteLogSegmentMetadataUpdate, RemoteLogSegmentState, RemotePartitionDeleteMetadata, RemotePartitionDeleteState, RemoteStorageManager}
...
class RemoteLogManager(fetchLog: TopicPartition => Option[Log],
  ...
  def onLeadershipChange(partitionsBecomeLeader: Set[Partition], partitionsBecomeFollower: Set[Partition], topicIds: Map[String, Uuid]): Unit = {
    ...
    topicPartitionIds = partitionIds.toMap
    ...
    if (delete) {
      try {
        ...
        topicIdPartition.foreach(idPartition => {
          remoteLogMetadataManager.listRemoteLogSegments(idPartition).forEachRemaining(deleteRemoteLogSegment)
  ...
  private def deleteRemoteLogSegment(metadata: RemoteLogSegmentMetadata): Unit = {
    try {
      ...
      val remotePartitionDeleteMetadata = new RemotePartitionDeleteMetadata(metadata.topicIdPartition(),
        RemotePartitionDeleteState.DELETE_PARTITION_MARKED, metadata.eventTimestampMs(), metadata.brokerId())
              ...
              sortedSegments.slice(0, index).foreach { segment =>
                ...
                try {
                  ...
                  val segmentData = new LogSegmentData(logFile.toPath, segment.lazyOffsetIndex.get.path,
                    segment.lazyTimeIndex.get.path, segment.txnIndex.path, producerIdSnapshotFile.toPath,
                    ByteBuffer.wrap(Files.readAllBytes(leaderEpochs.toPath)))
...
//                log.updateRemoteIndexHighestOffset(readOffset)
  ...
  def fetchRemoteLogSegmentMetadata(tp: TopicPartition,
    ...
    val topicIdPartition = topicPartitionIds.get(tp)
  ...
  def findOffsetByTimestamp(tp: TopicPartition, timestamp: Long, startingOffset: Long): Option[TimestampAndOffset] = {
    ...
    remoteLogMetadataManager.listRemoteLogSegments(topicPartitionIds(tp)).asScala.foreach(rlsMetadata =>
