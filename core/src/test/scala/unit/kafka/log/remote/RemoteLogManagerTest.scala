...
class RemoteLogManagerTest {
  ...
  val rsmConfig: Map[String, Any] = Map(
    RemoteLogManagerConfig.REMOTE_STORAGE_MANAGER_CONFIG_PREFIX + "url" -> "foo.url",
    RemoteLogManagerConfig.REMOTE_STORAGE_MANAGER_CONFIG_PREFIX + "timeout.ms" -> 1000L
  )
  ...
  def testRSMConfigInvocation(): Unit = {
    ...
    assertEquals(rsmConfig.size,
      rsmConfig.count { case (k, v) =>
        val keyWithoutPrefix = k.split(RemoteLogManagerConfig.REMOTE_STORAGE_MANAGER_CONFIG_PREFIX)(1)
  ...
  def testRLMConfig(): Unit = {
    ...
    props.put(RemoteLogManagerConfig.REMOTE_LOG_METADATA_MANAGER_CONFIG_PREFIX + key, "world")
    ...
    assertEquals(props.get(RemoteLogManagerConfig.REMOTE_LOG_METADATA_MANAGER_CONFIG_PREFIX + key), rlmmConfig.get(key))
